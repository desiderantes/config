...
final class ResolveSource {
    ...
    static private ValueWithPath findInObject(AbstractConfigObject obj, Path path, Node<Container> parents) {
        ...
        Node<Container> newParents = parents == null ? new Node<>(obj) : parents.prepend(obj);
    ...
    ResolveSource pushParent(Container parent) {
        ...
        if (pathFromRoot == null) {
            if (parent == root) {
                return new ResolveSource(root, new Node<>(parent));
            } else {
                if (ConfigImpl.traceSubstitutionsEnabled()) {
    ...
    static final class Node<T> {
        ...
        Node<T> reverse() {
            if (next == null) {
                ...
                Node<T> reversed = new Node<>(value);
                Node<T> i = next;
                while (i != null) {
    ...
    record ValueWithPath(AbstractConfigValue value, Node<Container> pathFromRoot) {
    ...
    record ResultWithPath(ResolveResult<? extends AbstractConfigValue> result, Node<Container> pathFromRoot) {
