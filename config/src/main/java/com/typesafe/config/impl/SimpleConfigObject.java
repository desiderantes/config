...
final class SimpleConfigObject extends AbstractConfigObject implements Serializable {
    ...
    protected SimpleConfigObject withOnlyPathOrNull(Path path) {
        ...
        if (next != null) {
            if (v instanceof AbstractConfigObject abstractConfigObject) {
                v = abstractConfigObject.withOnlyPathOrNull(next);
            } else {
    ...
    SimpleConfigObject withoutPath(Path path) {
        ...
        if (next != null && v instanceof AbstractConfigObject abstractConfigObject) {
            v = abstractConfigObject.withoutPath(next);
            Map<String, AbstractConfigValue> updated = new HashMap<>(
                    value);
            updated.put(key, v);
            ...
            Map<String, AbstractConfigValue> smaller = new HashMap<>(
                    value.size() - 1);
    ...
    SimpleConfigObject withValue(Path path, ConfigValue v) {
        ...
        if (next == null) {
            ...
            if (child instanceof AbstractConfigObject abstractConfigObject) {
                // if we have an object, add to it
                return withValue(key, abstractConfigObject.withValue(next, v));
            } else {
    ...
    public SimpleConfigObject replaceChild(AbstractConfigValue child, AbstractConfigValue replacement) {
        HashMap<String, AbstractConfigValue> newChildren = new HashMap<>(value);
    ...
    public boolean hasDescendant(AbstractConfigValue descendant) {
        ...
        for (AbstractConfigValue child : value.values()) {
            if (child instanceof Container container && container.hasDescendant(descendant))
                return true;
        }
    ...
    public Set<Map.Entry<String, ConfigValue>> entrySet() {
        ...
        HashSet<java.util.Map.Entry<String, ConfigValue>> entries = new HashSet<>();
        for (Map.Entry<String, AbstractConfigValue> e : value.entrySet()) {
